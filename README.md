...
- [`docs/rps-api-contract.md`](docs/rps-api-contract.md) — the verified Identity + Engine API contract (auth, `/transform` shape, error model) this server is built on.
- [`docs/searchable-encryption.md`](docs/searchable-encryption.md) — the searchable-encryption guide: the scheme/leakage ladder, and the PROPE search-band contract (`dependencyContext {method,min,max}`, case-insensitive collation).
